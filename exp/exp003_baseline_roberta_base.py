...
class ROBERTA_BASE(nn.Module):
    def __init__(self):
        ...
        self.robert = transformers.RobertaModel.from_pretrained(Config.model)
        self.drop = nn.Dropout(0.2)
        self.out = nn.Linear(768, 1)

    def forward(self, ids, mask):
        _,output = self.robert(ids, attention_mask=mask, return_dict=False)
